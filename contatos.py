...
def tratar_telefone(tel_raw):
    ...
    if tel_str.lower() == 'nan' or tel_str == '': return "", "Sem telefone"
    tel_limpo = tel_str.split('.')[0]
    tel_num = ''.join(filter(str.isdigit, tel_limpo))
    if len(tel_num) < 8: return "", tel_str
    ...
    if not df.empty and 'Data de Nascimento' in df.columns:
        ...
        if not df_aniver.empty:
            ...
            for idx, row in df_aniver.iterrows():
                ...
                with bc1:
                    if tel_num:
                        texto_aniver = urllib.parse.quote(f"Olá {nome.split()[0]}! Em nome do Samir Bestene e de toda a nossa equipe, desejo um feliz aniversário! Que sua vida seja repleta de alegrias, muita saúde e sucesso. É uma honra ter você caminhando ao nosso lado. A luta continua 🚀")
                ...
                st.markdown("")
        else: st.info("Nenhuma data de nascimento válida encontrada.")
    else: st.info("Coluna de data de nascimento não localizada na planilha.")
...
with aba2:
    ...
    if not df.empty and 'Bairro' in df.columns:
        ...
        for idx, row in filtrados.iterrows():
            ...
            tel_num, tel_exibicao = tratar_telefone(row.get('Telefone', ''))
            st.markdown(f"<div class='contato-card'><b>👤 {nome}</b><br>📍 <b>Bairro:</b> {bairro} | 📞 {tel_exibicao}</div>", unsafe_allow_html=True)
            bc1, bc2 = st.columns(2)
            with bc1:
            ...
            st.markdown("")
    else: st.info("Nenhum dado de bairro encontrado.")
...
with aba3:
    ...
    if not df.empty and 'Nome Completo' in df.columns:
        ...
        for idx, row in df_contatos.iterrows():
            nome, bairro = str(row.get('Nome Completo', 'Sem Nome')), str(row.get('Bairro', ''))
            tel_num, tel_exibicao = tratar_telefone(row.get('Telefone', ''))
            st.markdown(f"<div class='contato-card'><b>👤 {nome}</b><br>📍 Bairro: {bairro} | 📞 {tel_exibicao}</div>", unsafe_allow_html=True)
            bc1, bc2 = st.columns(2)
            with bc1:
                if tel_num: st.markdown(f"<a href='https://api.whatsapp.com/send?phone=55{tel_num}&text={urllib.parse.quote(f'Olá {nome.split()[0]}, tudo bem?')}' target='_blank' style='display: block; text-align: center; background-color: #25D366; color: white; padding: 6px; border-radius: 4px; text-decoration: none; font-size: 14px; font-weight: bold;'>💬 WhatsApp</a>", unsafe_allow_html=True)
                else: st.markdown("<div class='btn-disabled'>S/ Número</div>", unsafe_allow_html=True)
            ...
            st.markdown("")
    else: st.info("Nenhum contato encontrado.")

# --- ABA 4: MAPA ---
with aba4:
    st.subheader("🗺️ Dispersão de Apoiadores")
    st.markdown("Mapa estimado baseado nos municípios e bairros cadastrados.")
    
    if not df.empty and 'Bairro' in df.columns:
        # Base de Coordenadas de Rio Branco e Municípios do Acre
        coord_referencia = {
            # Municípios
        ...
        # Pega a coluna Municipio (se existir) ou joga vazio
        col_mun = None
        ...
        for idx, row in df.iterrows():
            ...
            # Se for fora de Rio Branco, ancora no município. Se for na capital, ancora no Bairro.
            ...
            # Espalhamento orgânico (jitter) para pinos não sobreporem
            espalhamento_geo = 0.005 # Cerca de 500 metros de dispersão
            latitudes.append(base_lat + np.random.normal(0, espalhamento_geo))
            longitudes.append(base_lon + np.random.normal(0, espalhamento_geo))
            
        mapa_df = pd.DataFrame({'lat': latitudes, 'lon': longitudes})
        st.map(mapa_df)
    else: st.warning("Não há dados suficientes de localidade para gerar a mancha do mapa.")
            ...
            for idx, row in ranking.iterrows():
                ...
                with st.expander(f"#{idx + 1} | {lider} - {qtd} pessoa(s) ({(qtd / total_cadastros) * 100:.1f}%)"):
                    for _, apoiado in df_lideres[df_lideres[col_indicacao] == lider].iterrows():
                        nome_ap, bairro_ap = str(apoiado.get('Nome Completo', 'Sem Nome')), str(apoiado.get('Bairro', ''))
                        ...
                        st.markdown(f"<div class='apoiador-lider'><b>{nome_ap}</b><br><span style='font-size: 14px; color: #a9b9cc;'>📍 {bairro_ap} | {tel_html}</span></div>", unsafe_allow_html=True)
        else: st.info("Ainda não há dados suficientes de indicações preenchidos.")
    else: st.warning("A coluna de indicação não foi encontrada na planilha. Verifique o formulário.")
